...
class voltorb_embed:
    ...
    async def flip(self, ctx):
        ...
        if 'row' in key[:3].lower() or 'col' in key[:3].lower():
            coins = self.vol.edit_all(f'boards/{ctx.author.id}.png', key[:3].lower(), key[4:].lower())
        else:
            if not key[1].isdigit():
                await ctx.send(f'The column parameter must be of an integer type, {ctx.author.mention}')
                return
        ...
        self.message = (await ctx.send(files=[thumb, board], embed = embed)).id
        if self.lose:
            self.rip = (await ctx.send(f'Oh no! You hit a voltorb, {ctx.author.mention} and got 0 coins!\nType `{prefix}vf resume` to continue.')).id
        if self.win:
            self.rip = (await ctx.send(f'Game clear, {ctx.author.mention}! You received {self.coins*int(coins[:-1])} Coins! Type `{prefix}vf advance` to advance to level {self.level + 1}')).id
            self.coins = 0
    ...
    async def resume(self, ctx):
        try:
            ...
            await ctx.send(f'You didn\'t lose your current match yet, {ctx.author.mention}. If you would like to restart, type `{prefix}restart`')
...
class voltorb_board:
    def __init__(self, dictionary, name):
        ...
        for i, j in zip(range(10, 1811, 360), range(5)):
            ...
            draw.text((2050, i + 180), str(self.board[j].count(0)), (0, 0, 0), font=font)
        bg.save(name)
            ...
            for i in range(5):
                if not self.flip[num - 1][i]:
                    if self.board[num - 1][i] == 0:
                        bg.paste(hl_voltorb_tile, ((i + 1) * 360 - 350, num * 360 - 350))
...
class VoltorbFlip(commands.Cog):
    ...
    async def flip(self, ctx):
        self.v1 = self.load(ctx.author.id)
        if self.v1.win:
            await ctx.send(f'You\'ve already won your current session, {ctx.author.mention}. Type `{prefix}advance` to proceed to the next level')
            return
        elif self.v1.lose:
            await ctx.send(f'You\'ve lost your current session, {ctx.author.mention}. Type `{prefix}resume` to continue')
            return
    ...
    async def resume(self, ctx):
        ...
        if self.v1.win:
            ...
            return
        if not self.v1.lose:
            await ctx.send(f'You\'ve not lost your current session yet, {ctx.author.mention}.')
            return
        self.v1.vol = voltorb_board(self.data[str(ctx.author.id)]['vol'], f'boards/{ctx.author.id}.png')
